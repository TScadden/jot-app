...
fun SleepScreen(
                ...
                confirmButton = {
                    TextButton(onClick = {
                        datePickerState.selectedDateMillis?.let { millis ->
                            ...
                            val formatted = sdf.format(java.util.Date(millis))
                            val sleepMins = state.historicalSleep.find { it.first == formatted }?.second
                            if (sleepMins != null) {
                                selectedCompareDate = formatted to sleepMins
                            }
                        }
                    ...
                    TextButton(onClick = {
                        dateRangePickerState.selectedEndDateMillis?.let { millis ->
                            ...
                            for (i in 0 until 7) {
                                ...
                                val sleepMins = state.historicalSleep.find { it.first == dStr }?.second
                                if (sleepMins != null) {
                                    sum += sleepMins
                                    count++
                                }
    ...
    Scaffold(
                ...
                actions = {
                    if (state.isConnected) {
                        IconButton(onClick = { viewModel.sync() }) {
                            Icon(androidx.compose.material.icons.Icons.Default.Sync, "Sync", tint = NotelTextSecondary)
                        }
        ...
        Column(
            ...
            verticalArrangement = if (!state.isConnected || (state.isLoading && state.historicalSleep.isEmpty())) Arrangement.Center else Arrangement.Top
        ) {
            if (!state.isConnected) {
                Icon(Icons.Default.Bed, null, tint = NotelPrimary, modifier = Modifier.size(64.dp))
                Spacer(Modifier.height(16.dp))
                Text("Health Connect is not linked.", color = NotelTextPrimary, fontSize = 18.sp, fontWeight = FontWeight.Bold)
                Spacer(Modifier.height(8.dp))
                Text("Connect your health data in Settings to view your sleep profile.", color = NotelTextSecondary, fontSize = 14.sp)
            } else if (state.isLoading && state.historicalSleep.isEmpty()) {
                ...
                if (state.errorMessage != null) {
                    Spacer(Modifier.height(8.dp))
                    Text(state.errorMessage!!, color = MaterialTheme.colorScheme.error, fontSize = 14.sp, textAlign = androidx.compose.ui.text.style.TextAlign.Center)
                    Spacer(Modifier.height(8.dp))
                }
                
                val sd = state.sleepData ?: com.notel.notel.ui.viewmodel.SleepData(0,0,0,0,0,0,0)
                
                GlassyCard(modifier = Modifier.fillMaxWidth()) {
                    Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth()) {
                         Icon(Icons.Default.Bed, null, tint = NotelPrimary, modifier = Modifier.size(64.dp))
                         Spacer(Modifier.height(16.dp))
                         Text("Time Asleep", color = NotelTextSecondary, fontSize = 14.sp)
                         val hours = sd.minutesAsleep / 60
                         val mins = sd.minutesAsleep % 60
                         Text("${hours}h ${mins}m", color = NotelTextPrimary, fontSize = 48.sp, fontWeight = FontWeight.Bold)
                         Spacer(Modifier.height(8.dp))
                         Text("Sleep Efficiency: ${sd.efficiency}%", color = NotelPrimary, fontSize = 14.sp, fontWeight = FontWeight.Medium)
                    }
                }
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                    GlassyCard(modifier = Modifier.weight(1f)) {
                        Column {
                            Text("Awake", color = NotelTextSecondary, fontSize = 12.sp)
                            Text("${sd.wakeMinutes}m", color = NotelTextPrimary, fontSize = 20.sp, fontWeight = FontWeight.Bold)
                        }
                    }
                    GlassyCard(modifier = Modifier.weight(1f)) {
                        Column {
                            Text("REM", color = NotelTextSecondary, fontSize = 12.sp)
                            Text("${sd.remMinutes}m", color = NotelTextPrimary, fontSize = 20.sp, fontWeight = FontWeight.Bold)
                        }
                    }
                }
                Spacer(Modifier.height(12.dp))
                ...
                Spacer(Modifier.height(24.dp))
                Text("Total time in bed: ${sd.timeInBed / 60}h ${sd.timeInBed % 60}m", color = NotelTextSecondary, fontSize = 14.sp)
                ...
                selectedCompareDate?.let { (dateString, pastSleepAsleep) ->
                    Spacer(Modifier.height(16.dp))
                    GlassyCard(
                        ...
                        Box(modifier = Modifier.fillMaxWidth()) {
                            ...
                            Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth().padding(top = 16.dp)) {
                                val currentSleepAsleep = if (compareMode == "Days") {
                                ...
                                val diff = currentSleepAsleep - pastSleepAsleep
                                ...
                                Text("$displayDate: ${pastSleepAsleep / 60}h ${pastSleepAsleep % 60}m avg", color = NotelTextPrimary, fontSize = 16.sp)
                                Text(if (compareMode == "Days") "Today: ${currentSleepAsleep / 60}h ${currentSleepAsleep % 60}m" else "This Week: ${currentSleepAsleep / 60}h ${currentSleepAsleep % 60}m avg", color = NotelTextPrimary, fontSize = 16.sp, fontWeight = FontWeight.SemiBold)
                                Spacer(Modifier.height(8.dp))
